...
import com.phoenix.blog.system.mapper.SysMenuMapper;
...
public class SysMenuServiceImpl extends ServiceImpl<SysMenuMapper, SysMenu> implements ISysMenuService {
    ...
    @Override
    public Result queryList(SysMenuRequest req) {
        // 1. 条件查询所有菜单
     ...
     * @return 系统菜单
     */
    private SysMenu childrenMenu(List<SysMenu> menuList, SysMenu parentMenu) {
        // 存放menu菜单对象的所有子菜单
        List<SysMenu> children = new ArrayList<>();
        // 循环所有菜单，判断是否为 menu 的子菜单
        for (SysMenu item : menuList) {
            // 判断每个菜单的parentId是否等于menu对象中的id
            if (item.getParentId().equals(parentMenu.getId())) {
                //当前的菜单为子菜单，递归查询是否当前子菜单是否为其他菜单的父菜单
                children.add(childrenMenu(menuList, item));
            }
        }
